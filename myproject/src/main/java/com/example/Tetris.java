...
public class Tetris {
    ...
    private static volatile boolean gameOver = false; // Game over flag
    ...
    public static void main(String[] args) throws InterruptedException {
        ...
        leaderboard.addEntry(name, getScore()); // Add the name and score to leaderboard
    ...
    public static boolean dropRandomShape(char[][] board) throws InterruptedException {
        ...
        while (true) {
            ...
            if (input == 4 && canMove(board, shape, x - 1, y))
                x--;
            else if (input == 6 && canMove(board, shape, x + 1, y))
                x++;
            else if (input == 5 && canMoveDown(board, shape, x, y - 1))
                y++;
            else if (input == 8) {
                char[][] rotatedShape = rotateShapeClockwise(shape);
                if (canMove(board, rotatedShape, x, y))
                    shape = rotatedShape;
            }
    ...
    static class InputHandler implements Runnable {
        ...
        public void run() {
            try {
                while (!gameOver) { // Stop input handling when the game is over
